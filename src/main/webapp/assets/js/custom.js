var App = function () {
    ...
    var modal = function (content_id, effect, midClick_) {
        $.magnificPopup.open({
            ...
            midClick: midClick_ // allow opening popup on middle mouse click. Always set it to true if you don't provide alternative source.
        });
    };

    return {
        open_modal: function (content_id, effect, midClick_) {
            modal(content_id, effect, midClick_);
        },
        validateForm: function (formId, fn) {
            if ($('#' + formId).valid()) {
                fn();
            } else {
                new PNotify({
                    title: 'Formulario invalido',
                    text: 'Faltan campos y/o son incorrectos',
                    shadow: '',
                    opacity: 1.0,
...
$.fn.paddingRight = function (character, length) {
    return this.each(function () {
        $(this).on('blur', function () {
            var s = this.value;
            if (!s || !character || s.length >= length) {
                this.value = s;
                return;
            }
            var max = (length - s.length) / character.length;
            for (var i = 0; i < max; i++) {
                s += character;
            }
            this.value = s;
...
$.fn.paddingLeft = function (character, length) {
    return this.each(function () {
        $(this).on('blur', function () {
            var s = this.value;
            if (!s || !character || s.length >= length) {
...
$.fn.caretEnd = function () {
    return this.each(function () {
        $(this).focus(function () {
            var elemLen = this.value.length;
            // For IE Only
            if (document.selection) {
                ...
                // Firefox/Chrome
                this.selectionStart = elemLen;
                this.selectionEnd = elemLen;
            }
        });
    });
};
...
$.fn.nextOnEnter = function () {
    var getTabbableFields = function (form) {
        var ret = [],
                inpts = Array.prototype.slice.call(form.querySelectorAll('input,textarea'), 0),
                i = inpts.length;
        var j = 0;
        inpts.sort(function (a, b) {
            var tab1 = parseInt(a.getAttribute('tabindex'), 10),
                    tab2 = parseInt(b.getAttribute('tabindex'), 10);
            if (isNaN(tab1))
                tab1 = 0;
            if (isNaN(tab2))
                tab2 = 0;
...
//        for (var k = 0; k < i;k++) {
//            var tabi = parseInt(inpts[k].getAttribute('tabindex'), 10),
//                    txtType = inpts[k].getAttribute('type');            
//            ret[j] = inpts[i];
//            j++;
//        }
        return inpts;
    ...
    return this.each(function () {
        ...
        $(this).keypress(function (e) {
            ...
            var fields = getTabbableFields($(obj).closest("form")[0]);
            if (!(obj.tagName && (obj.tagName.match(/input/i))) || obj.getAttribute('type').match(/submit/i)) {
                return true;
            }
            if (e.keyCode === 13) {
                ...
                for (var k = 0; k < fields.length; k++) {
                    if (k != fields.length - 1) {
                        if (parseInt(fields[k].getAttribute('tabindex'), 10) == tabi) {
                            fields[k + 1].focus();
                        }
                    }
                }
